...
const ServiceCard = ({ service, isFavorite, onToggleFavorite }) => {
  ...
  const handleViewDetails = () => {
    toast.success(`Opening ${name} details page`);
  ...
  const handleCallService = (e) => {
    e.stopPropagation();
    toast.success(`Calling ${name}...`);
  };
  return (
    <div className="group bg-white/95 backdrop-blur-lg rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden border border-purple-200/50 hover:scale-105 hover:-translate-y-2">
      {/* Service Image */}
      <div className="relative h-56 bg-gradient-to-br from-purple-100 to-indigo-200 overflow-hidden">
        <img
          src={image}
          alt={name}
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="space-y-2">
          <h3 className="text-xl font-bold text-slate-800 group-hover:text-purple-700 transition-colors duration-300 leading-tight">
            {name}
          </h3>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {specialties.length > 2 && (
            <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-xs font-medium border border-purple-200 shadow-sm">
        ...
        <div className="pt-4 border-t border-slate-200">
          <div className="flex space-x-3">
            <button
              ...
              className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-3 px-4 rounded-2xl hover:from-purple-700 hover:to-indigo-700 transition-all duration-300 font-semibold text-sm flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl group-hover:scale-105"
            >
